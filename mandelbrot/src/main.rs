...
/// Plot a rectangle of the Mandelbrot set into a buffer of pixels.
///
/// The `bounds` argument gives the width and height of the buffer `pixels`, which holds one grayscale pixel per byte.
/// The `upper_left` and the `lower_right` arguments specify points on the complex plane corresponding to the upper-left and lower-right corners of the buffer.
fn render(
    pixels: &mut [u8],
    bounds: (usize, usize),
...
/// Given the row and the column of a pixel in the output image, return the corresponding point on the complex plane.
///
...
///
/// Specifically, `s` should have been the form <left><sep><right>, where <sep> is the character given by the `separator` argument,
...
/// it took for `c` to leave the circle of radius two centered on the origin.
///
